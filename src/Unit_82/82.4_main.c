...
int main()
{
    ...
    {
        fp = fopen(ARCHIVE_NAME, "a+b");     // 아카이브 파일을 생성
        if (fp == NULL)                      // 파일 생성(열기)에 실패하면
            return -1;                       // 프로그램 종료

        // 새 아카이브 헤더 생성
        archive->header.magic = 'XR';        // 매직 넘버 XR 저장(리틀 엔디언에서는 XR로 저장됨)
        ...
        append(archive, "hello1.txt");    // hello.txt 파일 추가
        append(archive, "hello2.txt");    // hello.txt 파일 추가
        append(archive, "hello3.txt");    // hello.txt 파일 추가
    }
